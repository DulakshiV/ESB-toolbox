...
/*
    	private static void copyMediatorContext(TreeNode node, String messageID) {
    		int i=0;
    		if(node.getChildren()!=null){
    			while (i < node.getChildren().size()) {
    				copyMediatorContext(node.getChildren().get(i),messageID);
    			
    				 i++;
    			}
    		}
    		SuperMediator a=new SuperMediator();
   		    a.copy(node.getContents());
   		    node.setContents(a);
		...
		private static void getLastCurrentMediator(TreeNode node,MessageContext synCtx) {
    		...
    		TreeNode lastAddedNode;
    		 ...
    		 if(node.getChildren()!=null && node.getChildren().size()>0){
    				 lastAddedNode=node.getLastChild();
