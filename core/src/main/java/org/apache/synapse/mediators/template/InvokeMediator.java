...
public void addChild(MessageContext synCtx, Mediator m){
	SuperMediator  a  = new NestedMediator();
 	ArrayList<TreeNode> children = new ArrayList<TreeNode>(); 
 	TreeNode nonroot;
	//current.addChildTreeNodeMediator(m);
	...
	private boolean mediate(MessageContext synCtx, boolean executePreFetchingSequence) {
		...
		System.out.println("Invoke mediator current 99"+current);
if (synLog.isTraceOrDebugEnabled()) {
        ...
        System.out.println("Invoke mediator current 146"+current);
            boolean result = mediator.mediate(synCtx);
			if (result && executePreFetchingSequence) {
				ContinuationStackManager.removeReliantContinuationState(synCtx);
			}
			System.out.println("Setting ending time of mediator invoke msg id at line 152"+current.getContents().getMsgID());
