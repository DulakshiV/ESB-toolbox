...
/**
 * The Sequence mediator either refers to a named Sequence mediator instance or
 * is a *Named* list/sequence of other (child) Mediators
 * <p/>
 * If this instance defines a sequence mediator, then the name is required, and
 * an errorHandler sequence name optional. If this instance refers to another
 * (defined) sequence mediator, the errorHandler will not have a meaning, and if
 * an error in encountered in the referred sequence, its errorHandler would
 * execute.
 */
...
FlowContinuableMediator {
	...
	/**
	 * flag to ensure that each and every sequence is initialized and destroyed
	 * atmost once
	 */
	 ...
	 * execute the list of mediators (children) contained within this. If a
	 * referenced named sequence mediator instance cannot be found at runtime,
	 * an exception is thrown. This may occur due to invalid configuration of an
	 * erroneous runtime change of the synapse configuration. It is the
	 * responsibility of the SynapseConfiguration builder to ensure that dead
	...
	public boolean mediate(MessageContext  synCtx) {
		...
		if (synLog.isTraceOrDebugEnabled()) {
			synLog.traceOrDebug("Start : Sequence "
					+ (name == null ? (key == null ? "<anonymous" : "key=<"
							+ key) : "<" + name) + ">");
		...
		if (key == null) {
			...
			synLog.traceOrDebug("********** Printing current node from the begining of SequenceMediator:"
					+ synCtx.getCurrent().getContents().getMediatorName());
			...
			try {
				...
				// push the errorHandler sequence into the current message as
				// the fault handler
				if (errorHandler != null) {
					...
					if (errorHandlerMediator != null) {
						...
						synLog.auditWarn("onError handler : " + errorHandler
								+ " for sequence : " + name
								+ " cannot be found");
				...
				// Add a new SeqContinuationState as we branched to new
				// Sequence.
				ContinuationStackManager.addSeqContinuationState(synCtx,
						this.getName(), sequenceType);
				...
				if (result) {
					...
					// if flow completed remove the previously added
					// SeqContinuationState
					ContinuationStackManager.removeSeqContinuationState(synCtx,
							sequenceType);
				...
				if (synLog.isTraceOrDebugEnabled()) {
					...
					synLog.traceOrDebug("End : Sequence <"
				...
				if (result) {
					...
					else if(current.getParent().getContents().getMediatorName().contains("Clone")|| current.getParent().getContents().getMediatorName().contains("Iterate")){
						current.getContents().setEndTime(System.currentTimeMillis());
						TreeNode mainRoot=(TreeNode)synCtx.getProperty("Root");
				...
				if (isStatisticsEnable()) {
					boolean shouldReport = Boolean.parseBoolean(String
							.valueOf(synCtx
									.getProperty(SynapseConstants.OUT_ONLY)));
					if (!shouldReport) {
						shouldReport = !(Boolean
								.parseBoolean(String.valueOf(synCtx
										.getProperty(SynapseConstants.SENDING_REQUEST))));
					}
					if (shouldReport) {
						StatisticsReporter.reportForComponent(synCtx,
			...
			if (m == null) {
				...
				// Update the SeqContinuationState position with the mediator
				// position of
	...
	public boolean mediate(MessageContext synCtx,
		...
		Mediator errorHandlerMediator = null;
		// push the errorHandler sequence into the current message as the fault
		// handler
		if (errorHandler != null) {
			errorHandlerMediator = synCtx.getSequence(errorHandler);
			...
			if (errorHandlerMediator != null) {
				...
				synLog.auditWarn("onError handler : " + errorHandler
						+ " for sequence : " + name + " cannot be found");
			}
		}
		...
		if (!continuationState.hasChild()) {
			...
			// if children exists first mediate from them starting from
			// grandchild.
			...
			if (result) {
				// after mediating from children, mediate from current
				// SeqContinuationState
				/////////////////////////////////////////////////////////////////////////////////////
				synCtx.setCurrent(current);
				...
				result = super.mediate(synCtx,
						continuationState.getPosition() + 1);
	...
	public synchronized void init(SynapseEnvironment se) {
		if (!initialized) {
			...
			if (!isDynamic()) {
				// mark as available, if this is marked previously as
			...
			if (key != null) {
				if (key.getKeyValue() != null) {
					...
					SequenceMediator sequenceMediator = (SequenceMediator) se
							.getSynapseConfiguration().getSequence(
									key.getKeyValue());

					if (sequenceMediator == null
							|| sequenceMediator.isDynamic()) {
						// undefined or dynamic sequences are treated as
						// unavailable
						// in the environment.
						// At the time of their initialization, these will be
						// marked as available.
						se.addUnavailableArtifactRef(key.getKeyValue());
					}
				} else {
					// sequences referred by key-expressions are treated as
	...
	public synchronized void destroy() {
		if (initialized) {
			...
			if (key != null) {
				// Clearing unavailable sequence references added by this
				// sequence
				if (key.getKeyValue() != null) {

					SequenceMediator sequenceMediator = (SequenceMediator) synapseEnv
							.getSynapseConfiguration().getSequence(
									key.getKeyValue());

					if (sequenceMediator == null
							|| sequenceMediator.isDynamic()) {
						synapseEnv.removeUnavailableArtifactRef(key
								.getKeyValue());
					}

				} else {
					synapseEnv.removeUnavailableArtifactRef(key.getExpression()
							.toString());
				}
			}
		}
	 ...
	 * local registry
	 * 
	 * @return return the key which is used to pick the sequence definition from
	 *         the local registry
	 */
	public Value getKey() {
	...
	/**
	 * @param errorHandler
	 *            to used handle error will appear during the mediation through
	 *            this sequence
	 */
