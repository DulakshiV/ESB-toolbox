...
import org.apache.synapse.mediators.tree.*;
 ...
 * @see ListMediator
 */
public abstract class AbstractListMediator extends AbstractMediator implements
ListMediator {

	public Stack<String> myStack = new Stack<String>();
	TreeNode current;
	/** theack list of child mediators held. These are executed sequentially */
...
TreeNode parent;
	...
	public boolean mediate(MessageContext synCtx, int mediatorPosition) {
		...
		// if I have been explicitly asked to enable or disable tracing, set it
		// to the message
		...
		try {
			...
			synLog.traceOrDebug("**********Printing current node at the begining of AbstractListMediator : "
					+ current.getContents().getMediatorName());

			if (synLog.isTraceOrDebugEnabled()) {
				synLog.traceOrDebug("Sequence <" + getType() + "> :: mediate()");
				synLog.traceOrDebug("Mediation started from mediator position : "
						+ mediatorPosition);
			...
			if (contentAware) {
				try {
					RelayUtils.buildMessage(((Axis2MessageContext) synCtx)
			...
			for (int i = mediatorPosition; i < mediators.size(); i++) {
				...
				synLog.traceOrDebug("********** Printing the mediator to be executed from AbstractListMediator : "
						+ mediators.get(i).getType());
				
				///////////////////////////////////////////////////////////////////////////////////////////////////////////

				if (!MediatorData.hasList(mediators.get(i))) {
					synLog.traceOrDebug("********** Type of the Mediator is single, from AbstractListMediator : ");

					MediatorData.createNewSingleMediator(synCtx,
							mediators.get(i));
				}
				////////////////////////////////////////////////////////////////////////////////////////////////////////////
				if (!mediators.get(i).mediate(synCtx)) { // execute the mediator
				//	System.out.println(mediators.get(i).getType()+"failed"+current+synCtx.getCurrent()+synCtx.getCurrent().getContents().getMsgID());
				...
				///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
					if(current.getChildren()!=null && !"".equals(current.getChildren())&& ! current.getChildren().isEmpty()){

						if(!"".equals(current.getLastChild().getContents().getMediatorName()) && current.getLastChild().getContents().getMediatorName()!=null && current.getLastChild().getContents().getMediatorName().equals("Skipped")){
							
							synCtx.getCurrent().getChildren().remove(synCtx.getCurrent().getLastChild());
			}

						else {
							MediatorData.setEndingTime(current.getLastChild());
