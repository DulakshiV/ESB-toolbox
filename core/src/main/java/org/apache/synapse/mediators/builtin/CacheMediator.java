...
public class CacheMediator extends AbstractMediator implements ManagedLifecycle,
	...
	public void check(MessageContext synCtx) {
		System.out.println("Cache mediator check method");

		TreeNode nonroot;
		current = synCtx.getCurrent();
		int lastChild = current.getChildren().size() - 1;
		nonroot = current.getChildren().get(lastChild);
		nonroot.getContents().setType("sequence");
		// current.addChildTreeNodeMediator(nonroot);
		current = nonroot;
		System.out.println("!!!!!!!!!!adding non root to current:.");
...
//        }
    ...
    public boolean mediate(MessageContext synCtx, ContinuationState contState) {
        ...
        if (synLog.isTraceOrDebugEnabled()) {
            synLog.traceOrDebug("Cache mediator : Mediating from ContinuationState");
        }
